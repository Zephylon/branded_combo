...
**Q.** `알베르`와 특정 종족 패가 있습니다. `낙인의 기염`을 서치해서 전개하고 싶은데요?

**A.** 그러셔도 좋습니다. 다만, 해당 검색 기능은 지원하지 않습니다. 따라서, `낙인의 기염+특정종족`를 사용하여 `하얀 용의 낙윤`을 서치하셨다면, 전개 과정 중에서 `데스피아의 도화 알베르`를 사용하는지 체크하시기 바랍니다. 

---

**Q.** 분명 전개가 되는 패인데 전개법 추천이 없어요. 

**A.** 이 프로그램은 스프레드시트를 기반으로 제작된 프로그램입니다. 따라서, 특정 파츠(`더 비스테드 루벨리온`, `혁의 낙인` 등)를 사용한 전개 방법밖에 시트에 등록되지 않았다면 충분히 전개가 되는 패임에도 노출되지 않을 수도 있습니다. 
